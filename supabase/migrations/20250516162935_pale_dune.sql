/*
  ...
  1. Changes
    - Add function to automatically check alerts periodically
    - Add trigger to run check every 15 minutes
...
-- It will only actually run if 15 minutes have passed since the last check
SELECT cron.schedule(
  'check-alerts',  -- unique job name
  '* * * * *',    -- every minute
  'SELECT trigger_automated_checks()'
);